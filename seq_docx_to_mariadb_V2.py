...
사용 예:
  ...
  # DB 접속정보 직접 지정
  python seq_docx_to_mariadb.py ./answers \
      --host 127.0.0.1 --port 3306 --user user --password pass --database metadata
"""
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--host", default=os.getenv("MARIADB_HOST", "127.0.0.1"))
    p.add_argument("--port", type=int, default=int(os.getenv("MARIADB_PORT", "3306")))
    p.add_argument("--user", default=os.getenv("MARIADB_USER", ""))
    p.add_argument("--password", default=os.getenv("MARIADB_PASSWORD", ""))
    p.add_argument("--database", default=os.getenv("MARIADB_DATABASE", ""))
    return p
...
def validate_db_args(args: argparse.Namespace) -> None:
    missing = []
    if not args.user:
        missing.append("--user / MARIADB_USER")
    if not args.database:
        missing.append("--database / MARIADB_DATABASE")
    if missing:
        raise SystemExit("DB 접속정보가 부족합니다: " + ", ".join(missing))
